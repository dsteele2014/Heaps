...
class Graph():
	...
	def topo_sort(self):
		...
		def depth_first_visit(u, spacer=''): 
			...
			nonlocal time
			time += 1	
			log(spacer+'topological_visit({}) at time {}:00'.format( u.label, time ),3)
			u.discovery = time
			u.color = Vertex.GRAY
			for v in self.Adj[ u ]:
				if v.color == Vertex.WHITE:
	...
	def dag_shortest_path(self, source):
		...
		:type source: str
		"""
		sorted_list = Graph.topo_sort(self)
		Graph.initialize_single_source(self,Vertex(label = source))
		for u in sorted_list:
			for v in self.Adj[u]:
				Graph.relax(self,u,v)
